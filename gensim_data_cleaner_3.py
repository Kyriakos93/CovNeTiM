...
def clean_exclusions(text):
    ...
    if str(text) == 'nan':
        ...
        text = re.sub(
            r'^(last modified on |first published on )?(mon|tue|wed|thu|fri|sat|sun) \d{1,2} (jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec) \d{4} \d{1,2}\.\d{1,2}\s[a-zA-Z]{3,4}\b(\n|\r|\r\n)',
            '', text)
...
def clean_numbers(text):
    ...
    if str(text) == 'nan':
        ...
        print("Applying: Covid codename references..")
        ...
        # Currency reference
        # print("Applying: Currency references..")
        ...
        # text = re.sub(r'\d{1,3}((\,|\.)?\d{1,3}?)* ?(m|bn|million|billion|trillion)? ?(\$|\€|\£)', ' [COVCURRENCY] ',
        #               text)
        # text = re.sub(r'\d{1,3}((\,|\.)?\d{1,3}?)* ?(euro(s)?|dollar(s)?|pound(s)?|eur|EUR|usd|USD|gbp|GBP)',
        #               ' [COVCURRENCY] ', text)
        ...
        # print("Applying: Numbers having mg..")
        # text = re.sub(r'\d{1,3}((\,|\.)?\d{1,3}?)* ?(mg|g|milligrams|grams)\b', ' [COVDOSE] ', text)
        #
        # # Numbers having cm, centimeters, m, meters, km, kilometers
        # print("Applying: Numbers having cm..")
        # text = re.sub(
        #     r'\d{1,3}((\,|\.)?\d{1,3}?)* ?(mm|cm|m|km|millimeter(s)?|millimetre(s)?|centimeter(s)?|centimetre(s)?|meter(s)?|metre(s)?|kilometer(s)?|kilometre(s)?)\b',
        #     ' [COVMESAURES] ', text)
        # text = re.sub(r'\d+(\,\d+)? ?feet', ' [COVMESAURES] ', text)
        # text = re.sub(r'\d{1,2} ?inch(es)?', ' [COVMESAURES] ', text)

        # Time reference
        print("Applying: Time references..")
        text = re.sub(
            r'(\d{1,2}\:\d{2}( ?(a\.?m\.?|p\.?m\.?))?( ?[a-zA-Z]{3,4}((\-|\+)\d{1,2})?)?)|(\d{1,2}( ?(a\.?m\.?|p\.?m\.?)){1}( ?[a-zA-Z]{3,4}((\-|\+)\d{1,2})?)?)',
            ' [COVTIME] ', text)
        ...
        # Date reference
        print("Applying: Date references..")
        # TODO: INCLUDE ADDITIONAL PATTERNS (eg. 9/11, 11/2020) --> DONE
        text = re.sub(
            r'((J|j)anuary|(F|f)ebruary|(M|m)arch|(A|a)pril|(M|m)ay|(J|j)une|(J|j)uly|(A|a)ugust|(S|s)eptember|(O|o)ctober|(N|n)ovember|(D|d)ecember|(J|j)an\.?|(F|f)eb\.?|(M|m)ar\.?|(A|a)pr\.?|(M|m)ay\.?|(J|j)un\.?|(J|j)ul\.?|(A|a)ug\.?|(S|s)ep\.?|(O|o)ct\.?|(N|n)ov\.?|(D|d)ec\.?) \d{1,2}(st|nd|rd|th)?(\,? ((20\d{2})|(19\d{2})))?',
            ' [COVDATE] ', text)
        text = re.sub(
            r'((J|j)anuary|(F|f)ebruary|(M|m)arch|(A|a)pril|(M|m)ay|(J|j)une|(J|j)uly|(A|a)ugust|(S|s)eptember|(O|o)ctober|(N|n)ovember|(D|d)ecember){1} \d{1,2}(st|nd|rd|th)?(\, ((20\d{2})|(19\d{2})))?',
            ' [COVDATE] ', text)
        text = re.sub(
            r'\d{1,2}(st|nd|rd|th)? ((J|j)anuary|(F|f)ebruary|(M|m)arch|(A|a)pril|(M|m)ay|(J|j)une|(J|j)uly|(A|a)ugust|(S|s)eptember|(O|o)ctober|(N|n)ovember|(D|d)ecember|(J|j)an|(F|f)eb|(M|m)ar|(A|a)pr|(M|m)ay|(J|j)un|(J|j)ul|(A|a)ug|(S|s)ep|(O|o)ct|(N|n)ov|(D|d)ec)( ((20\d{2})|(19\d{2})))?',
            ' [COVDATE] ', text)
        text = re.sub(
            r'\d{1,2}(st|nd|rd|th)? ((J|j)anuary|(F|f)ebruary|(M|m)arch|(A|a)pril|(M|m)ay|(J|j)une|(J|j)uly|(A|a)ugust|(S|s)eptember|(O|o)ctober|(N|n)ovember|(D|d)ecember)',
            ' [COVDATE] ', text)
        text = re.sub(r'\d{1,2}\/(\d{4}|\d{1,2})', ' [COVDATE] ', text)
        text = re.sub(r'\d+(/)+\d+(/)+\d+', ' [COVDATE] ', text)

        # Year reference
        print("Applying: Year references..")
        text = re.sub(r'(1900|2000)s', ' [COVYEAR] ', text)
        text = re.sub(r'(20\d{2})|(19\d{2})', ' [COVYEAR] ', text)

        # Day reference
        print("Applying: Day references..")
        text = re.sub(r'((M|m)onday|(T|t)uesday|(W|w)ednesday|(T|t)hursday|(F|f)riday|(S|s)aturday|(S|s)unday)',
                      ' [COVDAY] ', text)

        # Month reference
        print("Applying: Month references..")
        text = re.sub(
            r'(J|j)anuary|(F|f)ebruary|(M|m)arch|(A|a)pril|(M|m)ay|(J|j)une|(J|j)uly|(A|a)ugust|(S|s)eptember|(O|o)ctober|(N|n)ovember|(D|d)ecember',
            ' [COVMONTH] ', text)

        # Age References
        print("Applying: Age references..")
        ...
        print("Applying: Other semantics on covid numeric references..")
        text = re.sub(r'\d+((\,|\.)\d+)? ?(percent|\%)', ' [COVPERCENTAGE] ', text)

        # Coronavirus Deaths references
        print("Applying: Deaths references..")
        text = re.sub(r'\d+( new)?( coronavirus)? death(s)?\b', ' [COVDEATHS] ', text)

        # Coronavirus Tests references
        print("Applying: Tests references..")
        text = re.sub(r'\d+( new)?( coronavirus| covid)? test(s)?', ' [COVTESTS] ', text)

        # Coronavirus Cases references
        print("Applying: Cases references..")
        text = re.sub(r'\d+((\,|\.)\d+)*( confirmed)?( coronavirus)? case(s)?', ' [COVCASES] ', text)

        # Coronavirus New Cases references
        print("Applying: New cases references..")
        text = re.sub(r'\d+((\,|\.)\d+)* new( confirmed)?( coronavirus| covid)?( confirmed)? case(s)?',
                      ' [COVNEWCASES] ', text)

        # Numbers references written in full-text
        print("Applying: Full-text numbers references..")
        text = re.sub(
            r'(one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|thirteen|fourteen|fifteen|sixteen|seventeen|eighteen|nineteen|twenty|thirdy|forty|fifty|sixty|seventy|eighty|ninety|hundred(s)?|thousand(s)?|million(s)?|first|second|third|fourth|fifth|sixth|seventh|eighth|ninth|tenth|eleventh|twelfth)\b',
            ' [COVNUMASWORD] ', text)

        # Number references in parentheses and brackets
        print("Applying: Brackets and Parentheses references..")
        text = re.sub(r'\[\d+(\.{1,3}|\,)?\d*\]', '[BRANUM]', text)
        text = re.sub(r'\(\d+(\.{1,3}|\,)?\d*\)', '[PARNUM]', text)

        # Other general numeric references
        print("Applying: General numeric references..")
        text = re.sub(r'\b\d{1,3}((\,|\.)?\d{1,3}?)*\b', ' [COVNUM] ', text)
        text = re.sub(r'\b\d+((\.|\,)\d+)*\b', ' [COVNUM] ', text)

        # Other number references fused in words
        print("Applying: References fused in words..")
...
# Read CSV File
print('■ Reading CSV File (' + filepath + ') into DataFrame..')

iterations = 0
for data_df in pd.read_csv(filepath, low_memory=False, chunksize=1000):
    iterations += 1
    print('■ Chunk iteration: ' + str(iterations))
    ...
    if result is None:
        result = data_clean
    # else:
    #     result = result.add(data_clean, fill_value=0)
...
if args.isGenPickEnabled:
    ...
    cv = CountVectorizer(stop_words='english')
    data_cv = cv.fit_transform(result.Content)
    data_dtm = pd.DataFrame(data_cv.toarray(), columns=cv.get_feature_names(), index=headlines)
    print(data_dtm)
    print('Done\t\t', end='')
...
if args.export:
    ...
    data_exp['Content'] = result.Content
